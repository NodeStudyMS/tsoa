...
export class ChatService {
  // 채팅방 생성
  public async createRoom(roomParams: ChatRoomCreationParams): Promise<ChatRoom> {
  ...
  // 메시지 저장
  public async saveMessage(messageParams: ChatMessageCreationParams): Promise<ChatMessage> {
    return await ChatMessage.create(messageParams);
  }

  // 특정 채팅방의 메시지 조회
  public async getMessagesByRoomId(roomId: string, limit: number = 50): Promise<ChatMessage[]> {
